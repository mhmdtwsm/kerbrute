...
## ⚠️ Fork Disclaimer & Custom Modifications
This is a custom fork of Kerbrute featuring an added **`--output-mode`** flag. This feature allows security testers and operators to write clean, script-friendly output files without cluttering or losing the noisy, color-coded terminal progress display.

### New Output Modes (`--output-mode`)
* **`default`**: Preserves the original Kerbrute behavior (includes timestamps, KDC info, and verbose prefixes like `[+] VALID USERNAME:` both on screen and in the output file).
* **`email`**: Keeps the terminal fully verbose and color-coded, but writes **only** clean credentials/users to the output file:
  * For user enumeration: `username@domain.com`
  * For password spraying/bruteforcing: `username@domain.com:password`
* **`user`**: Keeps the terminal fully verbose and color-coded, but writes **only** raw names to the output file:
  * For user enumeration: `username`
  * For password spraying/bruteforcing: `username:password`
...
Flags:
  ...
  -o, --output string       File to write logs to. Optional.
      --output-mode string  Clean output formats: default, email, or user (default "default")
